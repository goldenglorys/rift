...
def match_pattern(input_line, pattern):
    if pattern and pattern[0] == ANCHOR:
        # If the pattern starts with ^, we can only match at the start of the line
        if len(input_line) == 0 and len(pattern) == 1:
            return True
        if len(input_line) > 0 and pattern[1] == input_line[0]:
            return match_pattern(input_line[1:], pattern[1:])
        else:
            return False
    elif len(input_line) == 0 and len(pattern) == 0:
        return True
    elif not pattern:
        return True
    elif not input_line:
